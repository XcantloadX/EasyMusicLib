...
static EM_ERR lyric_init(Lyric *lyric, const char *str)
{
	...
	//分配空间
	lyric->length = count;
	lyric->time_array = calloc(count, sizeof(double));
	lyric->lyric_array = calloc(count, sizeof(char*));
	if (lyric->time_array == NULL || lyric->lyric_array == NULL)
		goto error_cleanup;
	...
	while (line != NULL)
	{
		...
		lyric->lyric_array[lyric_line_index] = strdup(text_line);
...
error_cleanup:
//	free(str_new);
	free(lyric->time_array);
	free(lyric->lyric_array);
...
Lyric *lyric_create_from_file(const char *file_path)
{
	...
	while (!feof(f))
	{
		if (index + 128 > len) //字符串长度自动扩容
			realloc(str, len *= 2);
		index += fread(str + index, sizeof(char), 128, f);
	}
	str[index] = '\0';
	lyric_init(lyric, str);
...
EM_ERR player_open(MusicPlayer *player, const char *file_path)
{
	...
	//params.lpstrElementName = L"D:\\Music\\陈致逸,HOYO-MiX - Genshin Impact Main Theme 原神.mp3";
...
{
	last_error = mciSendCommandA(player->device_id, MCI_CLOSE, 0, NULL);
	return EM_ERR_OK;
}
